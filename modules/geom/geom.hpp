...
namespace qpp{
  ...
  */
  ...
  struct periodic_cell :  public qpp_object{
    ...
    periodic_cell(const STRING & __name = "", qpp_object * __owner = NULL) : 
      qpp_object(__name,_owner)
    {}
  ...
  */
  ...
  public:
    ...
    {
      lace::vector3d<CREAL> c = crd[i.atom()];
      // fixme - kostyl to convert fractional coordinates into cartesian
      if (frac)
	c = trnf -> frac2cart(c);
      return trnf -> transform(c,i);
    }
  ...
  public:
	...
	*/
	...
	grainsize_auto = true;
      ...
      void auto_grain_size()
      {
	grainsize_auto = true;
      }

      void set_grain_size(CREAL gs)
      {
	grainsize_auto = false;
	grainsize = gs;
      }
