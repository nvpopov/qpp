...
namespace qpp{
  ...
  class bonding_table{
    ...
    inline bool _record_match(const STRING & at1, const STRING & at2, int i) const
    {
  ...
  public:
    ...
    py_indexed_property<SELF, REAL, const STRING &, &SELF::covrad, &SELF::set_covrad> py_covrad;

    REAL py_getpair(const char *){}
    void py_setpair(const char *, const REAL &){}

    REAL py_getpair2(const char *at1, const char *at2)
    {
      return pair(at1,at2);
    }

    void py_setpair2(const char *at1, const char *at2, const REAL & d)
    {
      set_pair(at1,at2,d);
    }

    py_2indexed_property<SELF,REAL,REAL, const char *, &SELF::py_getpair, &SELF::py_setpair, 
	...
	d[bp::make_tuple(i->at1, i->at2)] = i->d;

      return d;
    }

    void from_dict( bp::dict & d)
    {
      
      while (bp::len(d)>0)
	{
	  bp::tuple p = d.popitem();
	  bp::extract<REAL> val(p[1]);
	  if (!val.check())
	    TypeError("bonding_table::from_dict: Invalid dictionary, values must be of real type");
	  ...
	  else if (bp::extract<bp::tuple>(p[0]).check())
	    {
	      bp::tuple t = bp::extract<bp::tuple>(p[0]);
	      if (bp::len(t)!=2)
		TypeError("bonding_table::from_dict: Invalid dictionary, tuple key can contain a pair of atoms only");

	      bp::extract<STRING> at1(t[0]), at2(t[1]);
	      if (at1.check() && at2.check())
		set_pair(at1,at2,val);
	      else
		TypeError("bonding_table::from_dict: Invalid dictionary, tuple key can contain a pair of atoms only");
	    }
	  else
	    TypeError("bonding_table::from_dict: Invalid dictionary, key can be \'default\', an atom, or a pair of atoms");
	}
    }
    
    static void py_export(const char * pyname)
    {
      bp::class_<bonding_table<REAL> >(pyname, bp::init<>())
	.def_readwrite("default", &SELF::default_distance, "The default bonding distance between any atoms (r/w)" )
	.def_readwrite("covrad",  &SELF::py_covrad, "Covalent radius for atom. Usage: covrad[at] with string at(r/w)")
	.def_readwrite("pair",    &SELF::py_pair, 
		       "Pair specific bonding distance. Usage: pair[at1,at2] with string at1,at2 (r/w)")
	.def("distance", &SELF::distance, 
	     "Bonding distance between two atoms. Usage: distance(at1,at2) with string at1,at2")
	.def("to_dict", & SELF::to_dict, "Output the content of bonding table as dictionary. Usage: to_dict()")
	.def("from_dict", & SELF::from_dict, "Fill the bonding table from dictionary. Usage: from_dict(dict)")
	.def("clear", & SELF::clear, "Erase all data records")
	.def("merge",  & SELF::merge, "Merge another bonding table to this one")
	;
      py_indexed_property<SELF, REAL, const STRING &, &SELF::covrad, &SELF::set_covrad>::py_export("noname");
      py_2indexed_property<SELF,REAL,REAL, const char *, &SELF::py_getpair, &SELF::py_setpair, 
			   &SELF::py_getpair2, &SELF::py_setpair2 >::py_2export("noname",true);
    }
  ...
  class neighbours_table : public geometry_observer<REAL>{
    ...
    inline REAL distance(int i, int j)
    {
      return _disttable[i*ntp+j];
    }
  ...
  public:
    ...
    inline int n(int i) const
    {
      return _table[i].size();
    }

    // j-th neighbour of i-th atom
    inline index table(int i, int j) const
    {
      return _table[i][j];
    }
  ...
  private:
    ...
    inline std::vector<index> & grain(const index & I)
    {
      return grains[gidx(I)];
    }
    ...
    {
      index I=igrain(at);
      if ( I(0)>=0 && I(0)<ngrain(0) && I(1)>=0 && I(1)<ngrain(1) && I(2)>=0 && I(2)<ngrain(2) )
  ...
  public:
    ...
    void set_grain_size(REAL gs)
    {
      auto_grainsize = false;      
      grainsize = gs;
    }
      ...
      for (const auto & gp : gpairs)
	{
	  ...
	  for (int c1 = 0; c1 < grains[g1].size(); c1++)
		...
		if (!skip)
		  skip = at1.sub(1) != index::D(DIM).all(0) && at2.sub(1) != index::D(DIM).all(0);		
		if (skip)
		  continue;

		REAL r = norm(geom->pos(at1) - geom->pos(at2));
		
		//std::cout << "at1 = " << at1.atom << at1.cell << " grain " << I << " at2= "<< at2.atom << at2.cell << " grain " << I+DI <<  " DI =" << DI <<"\n"; 
    ...
    index py_getitem2(bp::tuple I) const
    { 
      if (bp::len(I)!=2)
	TypeError("neigbour_table::getitem. Expected 2 integers");
      if ( ! bp::extract<int>(I[0]).check() || ! bp::extract<int>(I[1]).check())
	TypeError("neigbour_table::getitem. Expected 2 integers");
    ...
    static void py_export(const char * pyname)
    {
      bp::class_<neighbours_table<REAL,CELL> >(pyname, bp::init< geometry<REAL, CELL> &, bonding_table<REAL> & >())
	.add_property("auto_update", &SELF::get_auto_update,  &SELF::set_auto_update,
		      "bool auto_update (default: False). If auto_update==True, any modifications made to atoms are\nautomatically reflected by neighbours_table")
	.def_readwrite("reference_mode", &SELF::reference_mode, 
		       "bool reference_mode (default: False). If reference_mode==True, simple but very inefficient\nalgorythm is used for building neighbours_table. For small molecules (less then 100 atoms) and for debugging purpose")
	.add_property("grain_size", &SELF::get_grain_size, &SELF::set_grain_size, 
	...
	.def(sn::self == sn::self)
	.def(sn::self != sn::self)
	.def("len", &SELF::n, "Usage: len(int i) - number of neighbours of i-th atom")
