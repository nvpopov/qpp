  ...
  void read_ccd_from_molden(std::basic_istream<CHAR_EX,TRAITS> & inp,
    ...
    while (!inp.eof()) {
        ...
        if (s.find(MOLDEN_FR_COORD) != std::string::npos) {
            if (!output.m_init_types.empty() &&
                tmp_displ.size() != output.m_init_types.size())
              tmp_displ.reserve(output.m_init_types.size());
        ...
        if (ps == molden_parser_state_e::molden_fr_norm_coords) {
            if (s.find(MOLDEN_VIBRATION) != std::string::npos) {
                norm_mode_id++;
                 if (!output.m_init_types.empty())
                  output.m_vibs[norm_mode_id].m_disp.reserve(output.m_init_types.size());
                continue;
              } else {
                std::vector<std::string_view> splt = split_sv(s, " ");
        ...
        if (ps == molden_parser_state_e::molden_atoms) {
            ...
            output.m_init_pos.push_back(std::move(atm_pos));
            output.m_init_types.push_back(std::string(splt[0]));
            continue;
          } // atoms after MOLDEN_ATOMS

      }

    output.m_tot_nat = output.m_init_types.size();
