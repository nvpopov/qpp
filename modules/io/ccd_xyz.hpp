...
void read_ccd_from_xyz_file(std::basic_istream<CHAR_EX,TRAITS> &inp,
  ...
  xyz_parser_state p_state{xyz_parser_state::state_atom_count};
  ...
  while (!inp.eof()) {
    ...
    if (p_state == xyz_parser_state::state_atom_count) {
      if (s.empty())
        break;
      cur_tot_at = str2int(cur_line, s);
      if (!init_filled)
        output.m_tot_nat = cur_tot_at;
      if (output.m_tot_nat != cur_tot_at) {
        nonconst_na = true;
        output.m_noncons_nat = true;
      }
    ...
    if (p_state == xyz_parser_state::state_comment) {
      ...
      if (!init_filled) {
        output.m_init_pos.resize(cur_tot_at); // will be filed with number of atoms from 0 frame
        output.m_init_types.resize(cur_tot_at);
      } else {
        ...
        if (frame_idx == 0) {
          ...
          output.m_steps[frame_idx].m_atom_pos = output.m_init_pos;
          output.m_steps[frame_idx].m_tot_nat = output.m_tot_nat;
          frame_idx +=1;
        }
        output.m_steps.resize(output.m_steps.size()+1);
        output.m_steps[frame_idx].m_atom_pos.resize(cur_tot_at);
        output.m_steps[frame_idx].m_tot_nat = cur_tot_at;
        if (nonconst_na) {
          output.m_steps[frame_idx].m_atom_types.resize(cur_tot_at);
        }
      }
      continue;
    ...
    if (p_state == xyz_parser_state::state_atom_data) {
      ...
      if (!init_filled) {
        output.m_init_types[atom_c] = std::string{splt[0]};
        output.m_init_pos[atom_c] = pos;
      } else {
        output.m_steps[frame_idx].m_atom_pos[atom_c] = pos;
        if (nonconst_na) {
          output.m_steps[frame_idx].m_atom_types[atom_c] = std::string{splt[0]};
        }
      }

      atom_c+=1;
      if (atom_c == cur_tot_at) {
        atom_c = 0;
        if (!init_filled) {
          init_filled = true;
        }
