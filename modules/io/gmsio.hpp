...
namespace qpp{
  ...
  void read_gms_basis(std::basic_istream<CHAR,TRAITS> & inp, atomic_basis<BT,FREAL> & bas)
  {
    bas.shells.clear();
    STRING s1, s2;
    std::getline(inp,s1);
  ...
  template<qpp_bastype BT, class FREAL>
  void write_gms_basis(std::basic_ostream<CHAR,TRAITS>  & out, const atomic_basis<BT,FREAL> & bas)
  {
	...
	if (sh.nshells()==1)
	  {
	    if (sh.l(0)==0)
	      ...
	      ValueError("Only S,P,D,F, and SP shells can be written in GAMESS format");
	  }
  ...
  void read_gms_ecp(std::basic_istream<CHAR,TRAITS>  & inp, STRING & name, atomic_ecp<FREAL> & ecp)
  {
    STRING s;
    std::getline(inp,s);
  ...
  template<class FREAL>
  void write_gms_ecp(std::basic_ostream<CHAR,TRAITS>  & out, const STRING & name, 
	...
	for (int l=0; l<ecp.L(); l++)
	  {
	    ...
	    for (int i=0; i<ecp.nprim(l); i++)
	      out << fmt::format("{:15.8f} {:3d} {:15.8f}",ecp.coeff(l,i),ecp.n(l,i),ecp.alpha(l,i)) 
		  << std::endl;
	  }
      }
  }
  ...
  void write_gms_data(std::basic_ostream<CHAR,TRAITS>  & out, geometry<REAL, CELL> & geom, 
		      std::vector<atomic_basis<qbas_gauss,REAL> > & bas)
  {
    if (!geom.is_xgeometry())
      TypeError("Must be xgeometry with charges to be printed as GAMESS data section");
  ...
  void write_gms_data(std::basic_ostream<CHAR,TRAITS>  & out, qmmm_molecule<REAL,CELL,qbas_gauss,FREAL> & mol)
  {
  ...
  template<class REAL, class CELL = periodic_cell<REAL> >
  void write_gms_charges(std::basic_ostream<CHAR,TRAITS>  & out, xgeometry<REAL, CELL> & geom)
  {
    out << " $EFRAG" << std::endl 
      ...
      if (!geom.shadow(i))
	{
	  ...
	  out << fmt::format("{:5d} {:16.8f} {:16.8f} {:16.8f}",i,r.x(),r.y(),r.z()) << std::endl;
	}
  ...
  template<class REAL, class CELL = periodic_cell<REAL>, class FREAL = REAL >
  void write_gms_charges(std::basic_ostream<CHAR,TRAITS>  & out, qmmm_molecule<REAL,CELL,qbas_gauss,FREAL> & mol)
  {
    for (int i : mol.fixedatoms)
      mol.geom->shadow(i,true);
    write_gms_data(out, *mol.geom, mol.basis);
