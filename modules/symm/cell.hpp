...
*/
  ...
  "r" makes sence for rotational symmetries
*/
  ...
  /*! \brief fractional to cartesian  transformation
   makes sence only for periodic translational cells
   @param[in] r The fractional coordinates. In the case DIM==2 the third
   coordinate ( z=r(2) ) is orthogonal to both translation vectors
...
*/
  ...
  inline vector3<REAL> operator()(int i) const { return v[i]; }
...
*/
  ...
  */
  ...
  inline void py_setitem(int i, const vector3<REAL> &a) {
    if (i < 0) i += DIM;
    if (i < 0 || i >= DIM) IndexError("cell: index out of range");
    v[i] = a;
  }
...
std::basic_ostream<_CharT, _Traits> &operator<<(
    std::basic_ostream<_CharT, _Traits> &__os, const periodic_cell<REAL> &cl) {
  std::basic_ostringstream<_CharT, _Traits> __s;
  __s.flags(__os.flags());
  __s.imbue(__os.getloc());
  __s.precision(__os.precision());
  __s << "cell(" << cl(0);
  for (int d = 1; d < cl.DIM; d++) __s << "," << cl(d);
  __s << ")";
  return __os << __s.str();
}
