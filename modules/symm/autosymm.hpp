...
bool has_symmetry(geometry<REAL, CELL> &geom, CELL &symm,
  ...
  bool res = true;
  int Ng = 1;
  for (int d = 0; d < symm.DIM; d++) Ng *= symm.end()(d) - symm.begin()(d) + 1;
  ...
  for (int i = 0; i < geom.nat(); i++) {
    ...
    for (int j = 0; j < ngbr.n(i); j++) {
      ...
      if (geom.atom(i) == geom.atom(J) && J != index::D(symm.DIM).atom(J(0))) {
        found++;
        // std::cout << "EQ";
      }
      if (found == Ng - 1) break;
 ...
 */
template <class REAL>
void bravais_point_group(
    array_group<matrix3<REAL>> &G, const periodic_cell<REAL> &cell,
  ...
  int fx = int(fmax(0)) + 1, fy = int(fmax(1)) + 1, fz = int(fmax(2)) + 1;
  for (iterator I({-fx, -fy, -fz}, {fx, fy, fz}); !I.end(); I++) {
    vector3<REAL> r = cell.transform({0., 0., 0.}, I);
    if (S.within(r)) points.add(std::string("point"), r);
  }
...
template <class REAL>
void find_translations(
    std::vector<vector3<REAL>> &transl, std::vector<permutation> &perm,
    geometry<REAL, periodic_cell<REAL>> &g1,
    geometry<REAL, periodic_cell<REAL>> &g2, const periodic_cell<REAL> &cell,
    REAL R = geometry<REAL, periodic_cell<REAL>>::tol_geom_default) {

  transl.clear();
  g1.build_types();
  g2.build_types();
  if (g1.nat() != g2.nat() || g1.n_types() != g2.n_types()) return;
  int nt = g1.n_types();
  ...
  if (!std::equal(t1.begin(), t1.end(), t2.begin())) return;

  // reduce to uc
  for (int i = 0; i < g1.nat(); i++) {
    if (g1.frac)
      for (int j = 0; j < 3; j++) g1.coord(i)(j) -= floor(g1.coord(i)(j));
    else
      g1.coord(i) = cell.reduce(g1.coord(i));

    if (g2.frac)
      for (int j = 0; j < 3; j++) g2.coord(i)(j) -= floor(g2.coord(i)(j));
    else
  ...
  for (int i = 0; i < t1[t].size(); i++) {
    ...
    for (int j = 0; j < g.nat(); j++) g.coord(j) += vs;
    ...
    for (int j = 0; j < g.nat(); j++) {
      ...
      for (iterator J({-1, -1, -1}, {1, 1, 1}); !J.end(); J++)
        for (int k = 0; k < g.nat(); k++)
          if ((g2.pos(k) - g.pos(j, J)).norm() < 2 * R &&
              g2.atom(k) == g.atom(j)) {
    ...
    if (is_transl) {
      ...
      for (int j = 0; j < g.nat(); j++) dv += g2.pos(j) - g.pos(j);
      dv /= g.nat();
...
template <class REAL>
void find_translations(
    std::vector<vector3<REAL>> &transl, geometry<REAL, periodic_cell<REAL>> &g1,
    geometry<REAL, periodic_cell<REAL>> &g2, const periodic_cell<REAL> &cell,
    REAL R = geometry<REAL, periodic_cell<REAL>>::tol_geom_default) {
...
/*! \brief Find the crystalline symmetry group
   ...
   */
template <class REAL>
void find_cryst_symm(
    array_group<rotrans<REAL, true>> &G,
...
/*! \brief Find the crystalline symmetry group
    @param G(OUT) - the crystalline group of bound rotranslational operations in
   positionary generator form. Bound rotranslational operations are used to make
   the group finite
    @param geom(IN) - the geometry of unit cell together with lattice vectors,
   which should be stored in geom.cell object. This geometry must be 3D -
   periodic
    @param R(IN) - the tolerance radius. Symmetry operation is considered valid,
   if the displacement of atom due to this operation is less than R
   */
template <class REAL>
void find_cryst_symm(
    genform_group<rotrans<REAL, true>> &G,
    geometry<REAL, periodic_cell<REAL>> &geom,
    REAL R = geometry<REAL, periodic_cell<REAL>>::tol_geom_default) {
...
std::vector<rotrans<REAL, false>> rotrans_circle(
    const rotrans<REAL, BOUND> &r) {
  rotrans<REAL, false> F(r.T, r.R), G = F;
  std::vector<rotrans<REAL, false>> circle = {G};
  while ((G.R - matrix3<REAL>::Identity()).norm() >
         rotrans<REAL, false>::tol_rot) {
    G = G * F;
    circle.push_back(G);
  }
  return circle;
...
std::vector<index> inspect_rotrans(const rotrans<REAL, true> &r, REAL eps) {
  std::vector<index> I = {index({1, 0, 0}), index({0, 1, 0}), index({0, 0, 1}),
                          index({0, 0, 0})};
...
bool is_subset(const std::vector<T> G1, const std::vector<T> G) {
  for (const T &i : G1)
    if (!belongs_set(i, G)) return false;
  return true;
}
...
void merge_to_set(std::vector<T> &A, const std::vector<T> &B) {
  for (const T &b : B)
    if (!belongs_to(b, A)) A.push_back(b);
}
...
void complete_group(std::vector<TRANSF> &S) {
  ...
  while (N.size() > 0) {
    ...
    for (const auto &i : N) S.push_back(i);
    N = NN;
  }
...
template <class REAL>
void fps_merge(std::vector<highsymmsite<REAL, rotrans<REAL, true>>> &H, int to,
...
/*! \brief Finds all point subgroups of crystalline symmetry group.
    ...
    @param subspaces (OUT) - std::vector containing the central points of the
   point groups
...
array_group<rotrans<REAL,true> > G8("",rotrans<REAL,true>(matrix3<REAL>::unity,
& cell8));
    ...
    for (int i=0; i<G8.size(); i++)
      for (int j=0; j<G8.size(); j++)
        if (A.multab(i,j)!=C.multab(i,j))
          std::cout << i << " " << j << " " << A.multab(i,j) << " " <<
C.multab(i,j) << std::endl;
...
//if (H[i].next.size()==0)
{
  /*
          ...
          for (const auto & j:H[i].group)
            std::cout << j << ",";
            std::cout << "} "
          std::cout << i << " dim= " << H[i].S.dim << " " <<
G[0].cell->reduce(H[i].S.point)
...
for (int i=0; i<subspaces.size(); i++)
  std::cout << i << " d= " << subspaces[i].dim << " pt= " << subspaces[i].point
<< " n= " << subspaces[i].axis
            << " ng= " << elements[i].size() << "\n";
...
/*
  for (int i=0; i<subspaces.size(); i++)
    {
      std::cout << "d= " << subspaces[i].dim << " pt= " << subspaces[i].pt << "
n= " << subspaces[i].n; for (int j=0; j<elements[i].size(); j++) std::cout <<
elements[i][j]; std::cout << "\n";
    }
...
py::list py_find_translations(geometry<REAL, periodic_cell<REAL>> &g1,
                              ...
                              const periodic_cell<REAL> &cell, REAL R) {

  std::vector<vector3<REAL>> t;
  find_translations(t, g1, g2, cell, R);
  py::list res;
  for (const auto &tt : t) res.append(tt);
  return res;
