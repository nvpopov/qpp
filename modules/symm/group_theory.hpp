...
public:
  ...
  group_analyzer( const static_table<int> & __multab) : multab(__multab), G(ARRAY()) {
    build_orders();
  }
...
public:
  ...
  group_characters(const ARRAY & _G, const group_analyzer<TRANSFORM,ARRAY> & _A) : G(_G), A(_A){
