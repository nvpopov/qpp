...
class shnfl {
 ...
 public:
  ...
  static array_group<matrix3<REAL> > Dnd(int n) {
    ...
    G.generate(RotMtrx<REAL>({0, 0, 1}, 2 * REAL(pi) / n));
    G.generate(RotMtrx<REAL>({1, 0, 0}, REAL(pi)));
    G.generate(Sigma<REAL>(
        {std::sin(REAL(pi) / (2 * n)), std::cos(REAL(pi) / (2 * n)), 0}));
    return G;
  }
  ...
  static array_group<matrix3<REAL> > I() {
    ...
    REAL alp =
        acos(-std::cos(3 * REAL(pi) / 5) / (1 + std::cos(3 * REAL(pi) / 5)));
    G.generate(
        RotMtrx<REAL>({std::sin(alp), 0, std::cos(alp)}, 2 * REAL(pi) / 5));
    return G;
  }
  ...
  static array_group<matrix3<REAL> > group(const STRING_EX &input_name) {

    STRING_EX name = tolower(input_name), error = "Nonexisting point group " + input_name;
    int n;
    size_t sz;
  ...
  static std::vector<STRING_EX> groups_by_order(int n) {
    if (n == 1)
      ...
      return {"Ci", "Cs", "C2", "C2h", "C2v", "D2", "D2h"};
    else if (n == 3)
      return {"T", "C3", "C3v", "D3"};
    else if (n == 4)
      return {"O", "Td", "C4", "C4h", "C4v", "D4", "D2d", "D4h", "S4"};
    else if (n == 5)
      return {"I", "C5", "C5v", "D5"};
    else if (n == 6)
      return {"Oh", "Th",  "C6",  "C3h", "C6h", "C6v",
              "D6", "D3d", "D3h", "D6h", "S6"};
    else if (n == 7)
      return {"C7", "C7v", "D7"};
    else if (n == 8)
      return {"C8", "C8h", "C8v", "D8", "D4d", "D8h", "S8"};
    else if (n == 9)
      return {"C9", "C9v", "D9"};
    else if (n == 10)
      return {"Ih",  "C10", "C5h", "C10h", "C10v",
              "D10", "D5d", "D5h", "D10h", "S10"};
    else {
      STRING_EX nn = std::to_string(n), n2 = std::to_string(n / 2);

      if (n % 2 == 1)
        return {"C" + nn, "C" + nn + "v", "D" + nn};
      else if (n % 4 == 2)
        return {"C" + nn,       "C" + n2 + "h", "C" + nn + "h",
                "C" + nn + "v", "D" + nn,       "D" + n2 + "d",
                "D" + n2 + "h", "D" + nn + "h", "S" + nn};
      else
        return {"C" + nn,       "C" + nn + "h", "C" + nn + "v", "D" + nn,
                "D" + n2 + "d", "D" + nn + "h", "S" + nn};
    }
  }
  ...
  struct fingerprint {
    ...
    bool compare(const fingerprint &f, REAL eps = matrix3<REAL>::tol_equiv) const {
      ...
      for (int i = 0; i < trace.size(); i++)
        if (!(std::abs(trace[i] - f.trace[i]) < eps &&
              std::abs(det[i] - f.det[i]) < eps && count[i] == f.count[i])) {
          res = false;
          break;
        }
    ...
    bool operator==(const fingerprint &f) const { return compare(f); }

    fingerprint() {}

    fingerprint(const array_group<matrix3<REAL> > &G,
    ...
    static void py_export(py::module &m, const char *pyname) {
      py::class_<shnfl<REAL>::fingerprint>(m, pyname)
          ...
          .def(py::init<const array_group<matrix3<REAL> > &, REAL>(),
               py::arg("G"), py::arg("eps") = matrix3<REAL>::tol_equiv)
          .def("add", &shnfl<REAL>::fingerprint::add, py::arg("tr"),
               py::arg("dt"), py::arg("eps") = matrix3<REAL>::tol_equiv)
          .def("compare", &shnfl<REAL>::fingerprint::compare, py::arg("f"),
