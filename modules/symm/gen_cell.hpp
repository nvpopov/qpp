...
struct gen_cell : public genform_group<TRANSFORM> {
  ...
  gen_cell(const std::vector<TRANSFORM> &g) : genform_group<TRANSFORM>(g) {
    init_default();
  }

  gen_cell(const genform_group<TRANSFORM> &G) : genform_group<TRANSFORM>(G) {
    init_default();
  }

  gen_cell(const gen_cell<REAL, TRANSFORM> &G) : genform_group<TRANSFORM>(G) {
    init_default();
  }

  inline vector3<REAL> transform(const vector3<REAL> &r, const index &I) const {
    return (*this)(I)*r;
  }
  ...
  // Does not make sense for gen_cell */
  inline vector3<REAL> reduce(const vector3<REAL> &r) const { return r; }
  ...
  vector3<REAL> symmetrize(const vector3<REAL> &r, REAL radius) const {
    ...
    for (const vector3<REAL> &pt : points) res += pt;
    res /= points.size();
    return res;
  }
  ...
  gen_cell(const py::list &G, const index &__begin, const index &__end)
  ...
  py_indexed_property<SELF, TRANSFORM, int, &SELF::py_getgen, &SELF::py_setgen>
      py_gen;

  TRANSFORM py_call(index I) { return (*this)(I); }
  ...
  py::list py_generate() {
    ...
    generate(v);
    py::list l;
    for (const auto &g : v) l.append(g);
    return l;
  }

  static void py_export(py::module m, const char *pyname) {
    std::string indexPropName =
        fmt::format("{0}_{1}", pyname, "indexed_property");
