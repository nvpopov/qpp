...
//   template<typename VALTYPE>
//   void vector_expression<VALTYPE>::process_expr(vector_expression<VALTYPE>* &target, 
// 						vector_expression<VALTYPE> *xpr)
//   /*  target is an object where the calculation result should be placed
//       target can possibly be:
//              1) actual vector
//              2) subvector of actual matrix
//              3) protovector that will be replaced with actual vector during calculation
//   */
//   {
...
//     // Create equality for the convenience of transformations, if we are not already in equality
//     vector_expression<VALTYPE> *equ;
//     bool equ_created = false;
//     if (   xpr -> gettype() & _equals  )
//       equ = xpr;
//     else
//       {
// 	equ = new typename vector_expression<VALTYPE>::binop( *target, *xpr, _equals);
//       //			     SUMMA<VALTYPE,matrix_expression<VALTYPE> >(*xpr), _equals);
// 	equ_created = true;
//       }
...
//     abstract_expression *xequ = equ;
//     canonical_expr<VALTYPE>(xequ);
//     equ = (vector_expression<VALTYPE>*)xequ;

//     // Perform calculation
//     ((vector_expression<VALTYPE>*)(equ -> xnext(1))) -> calculate_expr(target,true);
...
//   template<typename VALTYPE>
//   void matrix_expression<VALTYPE>::canonical_form_old( matrix_expression<VALTYPE>* &xpr )
//   {

//     std::cout << "Canonicalization started:\n";
//     xpr->print_tree(); 
//     std::cout << "\n";

//     // Create a holder object and the reduce the expression tree, removing equivalent nodes
//     matrix_expression<VALTYPE> *holder = new unop(*xpr,_link);
//     holder -> reduce_tree();

//     std::cout<< "After reduction:\n";
//     holder -> print_tree(1);

//     // Define literals for creating paternns
...
//     #define SUMM SUMMA<VALTYPE,matrix_expression<VALTYPE> >
//     #define SUMV SUMMA<VALTYPE,vector_expression<VALTYPE> >
//     #define trnf transform<VALTYPE>

//     // ------------- Starting transformations: -----------------------

//     // Get rid of minuses
//     transform<VALTYPE>( a - b , a + (VALTYPE(-1))*b ) . replace_all(holder);


//     std::cout << "Alive\n";

//     // Transform all pluses into sums
...
//       {
// 	contin = false;
// 	if ( trnf(  a + b                  , SUMM(a,b)                 ) . replace_once(holder) ) contin = true;
...
// 	  trnf equals( * new typename matrix_expression<VALTYPE>::binop( a, b,       _equals), 
// 		       * new typename matrix_expression<VALTYPE>::binop( a, SUMM(b), _equals) );
// // 	  equals.debug();
// 	  if ( equals.search(holder) )
// 	    if ( ! (b.bound() -> gettype() & _summa) )
// 	      equals.replace(holder);
// // 	      {
// // 		std::cout << "eq found: ";
// // 		holder -> print_tree();
// // 		std::cout << "\na = "; a.bound() -> print_tree();
// // 		std::cout << "\nb = "; b.bound() -> print_tree();
// // 		std::cout << "\n";
// // 	      }
// 	}
...
// 	  {
// 	    contin1 = false;
// 	    if ( trnf( SUMM( M1, a + b, M2 )  , SUMM( M1, a, b, M2)      ) . replace_once(holder) ) contin1 = true;
// 	    if ( trnf( a + SUMM( M1 )         , SUMM( a, M1 )            ) . replace_once(holder) ) contin1 = true;
// 	    if ( trnf( SUMM( M1 ) + a         , SUMM( M1, a )            ) . replace_once(holder) ) contin1 = true;
// 	    if ( trnf( SUMM( M1, s*(a+b), M2) , SUMM( M1, s*a, s*b, M2 ) ) . replace_once(holder) ) contin1 = true;
// 	    if ( trnf( s1*(s2*a)              , (s1*s2)*a                ) . replace_once(holder) ) contin1 = true;
// 	    if ( trnf( SUMM( M1, s1*a, M2, s2*a, M3 ) , 
// 		     SUMM( M1, (s1+s2)*a,  M2, M3 )  ) . replace_once(holder) ) contin1 = true;

// 	    if ( trnf( (s*a).T()              ,  s*a.T()                 ) . replace_once(holder) ) contin1 = true;

// 	    if ( trnf( SUMM( M1, s*SUMM(a).T(), M3 ) ,     
// 		     SUMM( M1, s*a.T(), M3 ) ) . replace_once(holder) ) contin1 = true;

// 	    if ( trnf( SUMM( M1, s*SUMM(a,M2).T(), M3 ) ,     
// 		     SUMM( M1, s*a.T(), s*SUMM(M2).T(), M3 ) ) . replace_once(holder) ) contin1 = true;
// 	    if ( trnf( (a.T()).T()    ,  VALTYPE(1)*a                      ) . replace_once(holder) ) contin1 = true;


//  	    if ( trnf( (s*a).H()              ,  conj(s)*a.H()           ) . replace_once(holder) ) contin1 = true;
// 	    if ( trnf( SUMM( M1, s*SUMM(a).H(), M3 ) ,     
// 		     SUMM( M1, s*a.H(), M3 ) ) . replace_once(holder) ) contin1 = true;
// 	    if ( trnf( SUMM( M1, s*SUMM(a,M2).H(), M3 ) ,     
// 		     SUMM( M1, s*a.H(), s*SUMM(M2).H(), M3 ) ) . replace_once(holder) ) contin1 = true;
// 	    if ( trnf( (a.H()).H()    ,  VALTYPE(1)*a                      ) . replace_once(holder) ) contin1 = true;


// 	    while ( trnf( SUMM( M1, s*SUMM(a,M2), M3 ),  SUMM( M1, s*a, s*SUMM(M2), M3) ) . replace_once(holder) ) 
// 	      contin1 = true;
// 	    if ( trnf( SUMM( M1, s*SUMM(), M3 ), SUMM( M1, M3)             ) . replace_once(holder) ) contin1 = true;

// 	    if ( trnf( SUMM( M1, SUMM(M2), M3 ), SUMM( M1, M2, M3)         ) . replace_once(holder) ) contin1 = true;

// 	    // fixme
// 	    // Temporary measure -- while transform.search(holder) does not properly work
// 	    // with summa literals

// 	    a.setmask( (~_scaling) ^ _basetypes );
	    
// 	    trnf( SUMM( M1, a, M2), SUMM( M1, VALTYPE(1e0)*a, M2) ) . replace_all(holder);
...
// 	    // Multiplications:
// 	    if ( trnf( SUMM( M1, s*( (s1*a) * b ), M2 ),  SUMM( M1,(s*s1)*(a*b), M2) ) . replace_once(holder) ) contin1 = true;
// 	    if ( trnf( SUMM( M1, s*(  a * (s1*b)), M2 ),  SUMM( M1,(s*s1)*(a*b), M2) ) . replace_once(holder) ) contin1 = true;
...
//       trnf scal( s*a );
//       abstract_expression *scaling = NULL;
//       while( scaling = scal.search(holder,scaling) )
// 	holder -> relink( scaling -> xnext(0), 
// 			  new scalar_number<VALTYPE>( ((scalar_expression<VALTYPE>*)(scaling -> next(0)))->value() ));
//     }
    

// //     abstract_expression::iterator<abstract_expression> ss(holder);
// //     while(ss != NULL)
// //       {
// // 	if ( ss -> basetype() == _scalar && !( ss -> gettype() & _actual) )
// // 	  {
// // 	    holder -> relink( &(*ss), new scalar_number<double>( ((scalar_expression<double>*)&(*ss))->value() ) );
// // 	    ss.set(holder);
// // 	  }
// // 	ss++;
// //       }
...
//     // 
//     // Now take what holder object is pointing to, and delete holder

//     #undef SUMM
//     #undef SUMV
//     #undef trnf

//     xpr = (matrix_expression<VALTYPE>*) holder -> next(0);
//     delete holder;
