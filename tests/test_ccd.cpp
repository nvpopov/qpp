...
TEST_CASE( "Computational chemistry data parsing : PC Gamess Firefly" ) {
  SECTION ("Parsing results of single-point calculations[PC Gamess Firefly]") {
    ...
    REQUIRE(cc_o.m_init_pos.size() == 20);
    REQUIRE(cc_o.m_init_types.size() == 20);
    REQUIRE(cc_o.m_init_pos[0][0] == Approx(2.6342558126*bohr_to_angs));
    REQUIRE(cc_o.m_init_pos[0][1] == Approx(-0.4183391645*bohr_to_angs));
    REQUIRE(cc_o.m_init_pos[0][2] == Approx(0.000));
  ...
  SECTION ("Parsing results of geometry optimization[PC Gamess Firefly]") {
    ...
    REQUIRE(cc_o.m_steps.front().m_atom_pos.size() == cc_o.m_init_pos.size());
    REQUIRE(cc_o.m_steps.back().m_atom_pos.size() == cc_o.m_init_pos.size());
    REQUIRE(cc_o.m_steps.back().m_atom_pos[0][0] == Approx(0.2292900177));
    REQUIRE(cc_o.m_steps.back().m_atom_pos[0][1] == Approx(1.3927230046));
    REQUIRE(cc_o.m_steps.back().m_atom_pos[0][2] == Approx(0.0));
    REQUIRE(cc_o.m_steps.back().m_atom_pos.back()[0] == Approx(-0.0328883188));
    REQUIRE(cc_o.m_steps.back().m_atom_pos.back()[1] == Approx(4.8736185925));
    REQUIRE(cc_o.m_steps.back().m_atom_pos.back()[2] == Approx(0.0));
    REQUIRE(cc_o.m_steps.back().m_atom_grads.front() == v3d(-0.0000150, -0.0000011, 0.0000000));
    REQUIRE(cc_o.m_steps.back().m_atom_grads.back() == v3d(-0.0000191, 0.0000149, 0.0000000));
  }
...
TEST_CASE("Testing parsing xyz files with ccd approach") {
  ...
  // Case from ASM, animation consists of frames with different number of atoms
  SECTION("Non-constant number of atoms") {
    std::ifstream isec("../examples/io/ref_data/xyz/defclusters.xyz");
    comp_chem_program_data_t<double> cc_o;
    read_ccd_from_xyz_file(isec, cc_o);
    bool compile_ccd_status = compile_ccd(cc_o, ccd_cf_default_flags);
    REQUIRE(compile_ccd_status == true);
    REQUIRE(cc_o.m_steps.size() == 16);
    REQUIRE(cc_o.m_tot_nat == 52); // from first frame
    std::array<size_t, 7> num_atoms = {52, 80, 59, 19, 84, 37, 45};

    for (int i = 0; i < 7; i++)
      REQUIRE(cc_o.m_steps[i].m_tot_nat == static_cast<int>(num_atoms[i]));

    std::vector<geom_anim_record_t<double> > anim_rec;
    bool succes_static = compile_static_animation(cc_o, anim_rec);
    bool succes_anims = compile_animation(cc_o, anim_rec);
    REQUIRE(succes_anims);
    REQUIRE(succes_static);
    REQUIRE(anim_rec.size() == 2);
    REQUIRE(static_cast<int>(anim_rec[0].frames[0].atom_types.size()) == cc_o.m_tot_nat);
    for (int i = 0; i < 7; i++) {
      REQUIRE(anim_rec[1].frames[i].atom_types.size() == num_atoms[i]);
      REQUIRE(anim_rec[1].frames[i].atom_pos.size() == num_atoms[i]);
      REQUIRE(anim_rec[1].frames[i].tot_nat == static_cast<int>(num_atoms[i]));
    }
    //REQUIRE(cc_o.m_tot_nat == 49651);
  }
}
...
TEST_CASE( "Computational chemistry data parsing : CP2K Output" ) {
  SECTION ("Parsing results of geometry optimization [CP2K]") {
    ...
    REQUIRE(cc_o.m_init_types.size() == 96);
    REQUIRE(cc_o.m_init_pos.size() == 96);
    REQUIRE(std::count(cc_o.m_init_types.begin(), cc_o.m_init_types.end(), "Ba") == 32);
    REQUIRE(std::count(cc_o.m_init_types.begin(), cc_o.m_init_types.end(), "F") == 64);
  ...
  SECTION ("Parsing results of geometry optimization [CP2K] - file 2") {
    ...
    bool succes_ccd_compilation =
        compile_ccd(cc_o, ccd_cf_default_flags | ccd_cf_remove_empty_geom_steps);
    REQUIRE(succes_ccd_compilation);
    REQUIRE(cc_o.m_steps.size() == 24);
  }
  ...
  SECTION ("Parsing results of geometry optimization [Orca] - file 1") {
    ...
    REQUIRE(cc_o.m_steps[0].m_atom_grads.size() == 24);
    REQUIRE(cc_o.m_steps[0].m_atom_grads[0][0] == Approx(0.076433267));
    REQUIRE(cc_o.m_steps[0].m_atom_grads[0][1] == Approx(-0.018518718));
    REQUIRE(cc_o.m_steps[0].m_atom_grads[0][2] == Approx(-0.066490034));
    REQUIRE(cc_o.m_steps[10].m_atom_grads.size() == 24);
