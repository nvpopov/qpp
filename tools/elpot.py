...
def mkshells(ns,maxs):
    psame = .3
    s = list(range(ns*maxs))
    shells =  [[s.pop(0) for i in range(random.randint(1,maxs))] for j in range(ns)]
    supshells = [[]]
    for i in range(ns):
        supshells[-1].append(i)
        if len(supshells[-1])<2 or ns-i<3:
            continue
        if random.random()<psame:
            supshells.append([])
    #try: supshells.remove([])
    #except ValueError: pass  
    return shells, supshells
...
def shellsolve(g,y,lam,L,shl,sups):
    m = y.shape[0]
    n = g.nat() - m
    ns = len(shl)
    nsups = len(sups)
    ...
    GS = glueG(G,shl)
    A = mkA(g,L)
    AS = glueA(A[:,:n],shl,sups)
    Q = np.zeros(shape=(AS.shape[0]))
    Q[:(L+1)**2] = - np.matmul(A[:,n:],y)
    ASQ = np.insert(AS,AS.shape[1],values=Q,axis=1)

    #print(AS)
    #print(Q)
    #print(ASQ)

    ASQ=np.array(LI_vecs(ASQ,smalleps))
    ...
    return lin_cond_min2(AS,FS,GS,Q,y)


def unroll(q,shl):
    n = max([i for s in shl for i in s])+1
